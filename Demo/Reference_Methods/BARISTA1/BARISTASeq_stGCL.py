...
utils.Cal_Spatial_Net(adata, rad_cutoff=50)
adata = train_model.train(adata,knn=6,n_epochs=epoch,use_image = use_image)
adata = utils.mclust_R(adata, used_obsm='stGCL', num_cluster=6)
...
print('%.5f ---------------------------------------------------------' %ARI)
# adata.obs.to_csv("../results/Seq/domains/slice3xxxx_stgcl_{}_Seq_{:.2f}.csv".format(seed,ARI))
