...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='X_pca', random_seed=666):
    ...
    adata.obs[f'MENDER_mclust_k{num_cluster}'] = mclust_res
    adata.obs[f'MENDER_mclust_k{num_cluster}'] = adata.obs[f'MENDER_mclust_k{num_cluster}'].astype('int')
    adata.obs[f'MENDER_mclust_k{num_cluster}'] = adata.obs[f'MENDER_mclust_k{num_cluster}'].astype('category')
    return adata
